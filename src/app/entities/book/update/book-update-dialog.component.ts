...
export class BookUpdateDialogComponent implements OnInit {
  ...
  save() {
    ...
    debugger;
    this.isSaving = true;
    const bookFormData = this.creatAndEditBookForm?.getRawValue() as IBook | NewBook;

    if (bookFormData.id) {
      this.bookService.update(bookFormData)
      ...
      this.bookService.create(bookFormData as NewBook)
        .pipe(finalize(() => (this.isSaving = false)))
        .subscribe({
          next: (res) => {
            if (res.ok) {
